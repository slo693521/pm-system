...
def check_password():
    ...
    st.title("🔒 存取受限")
    st.text_input("本 App 僅供授權人員使用，請輸入訪問密碼：", type="password", on_change=password_entered, key="password")
...
<style>
  ...
  .section-header {
    background: linear-gradient(90deg, #0d2137, #1a3a5c); color: white; padding: 10px 16px; border-radius: 6px;
...
COL_CONFIG = {
    ...
    "tracking": st.column_config.TextColumn("備註", width="medium"), "drawing": st.column_config.TextColumn("製造圖面"),
    "pipe_support": st.column_config.TextColumn("管撐製作"), "welding": st.column_config.TextColumn("研磨點焊"),
    "nde": st.column_config.TextColumn("焊道NDE"), "sandblast": st.column_config.TextColumn("噴砂"),
    "assembly": st.column_config.TextColumn("組立*"), "painting": st.column_config.TextColumn("噴漆"),
    "pressure_test": st.column_config.TextColumn("試壓"), "handover": st.column_config.TextColumn("交站"),
    "handover_year": st.column_config.SelectboxColumn("年份", options=["","114","115","116"]), "contact": st.column_config.TextColumn("對應窗口"),
...
with page_tab1:
    ...
    with btn_cols[0]:
        ...
        if st.button("📋 全部" + (" ✓" if is_all else ""), use_container_width=True, type="primary" if is_all else "secondary"):
    ...
    for i,(key,cfg) in enumerate(STATUS_CONFIG.items()):
        ...
        if st.button(f"{cfg['icon']} {cfg['label']} ({count})" + (" ✓" if active else ""), use_container_width=True, type="primary" if active else "secondary"):
    ...
    if not df.empty:
        ...
        if search:
            mask = (df["project_name"].str.contains(search,na=False) | df["case_no"].str.contains(search,na=False) | df["client"].str.contains(search,na=False) | df["contact"].str.contains(search,na=False))
            df = df[mask]
    ...
    for sec in sections_to_show:
        ...
        if not df_sec.empty:
            ...
            for k,cfg in STATUS_CONFIG.items():
                n = int(cts2.get(k,0))
                if n: badges += f'<span style="background:{cfg["btn"]};color:{cfg["text"]};border-radius:10px;padding:1px 9px;font-size:11px;margin-left:6px;font-weight:700;">{cfg["label"]} {n}</span>'
        ...
        styled = (show_df.assign(status_type=df_sec["status_type"].values).style.apply(color_rows,axis=1).format(na_rep=""))
        st.dataframe(styled, use_container_width=True, hide_index=True, height=min(420,38+len(df_sec)*35), column_config={k:v for k,v in COL_CONFIG.items() if k in show_df.columns})

        with st.expander(f"✏️ 編輯【{sec}】"):
            edit_df = df_sec[[c for c in DISPLAY_COLS+["status_type"] if c in df_sec.columns]].copy()
            edit_df["id"] = df_sec["id"].values # 保留 ID 供儲存使用
        ...
        if st.button("💾 儲存所有變更", type="primary", use_container_width=True):
            with st.spinner("🚀 啟動極速儲存模式..."):
                try:
                    ...
                    for sec, (old_df, new_df) in edited_data.items():
                        for i, row in new_df.iterrows():
                            ...
                            if "id" in old_df.columns and i < len(old_df):
                                ...
                                old_status = str(old_df.iloc[i].get("status_type", ""))
                                new_status = str(row.get("status_type", ""))
                                
                                # 如果所有格子和狀態都沒變，直接略過這筆不儲存 (節省99%時間)
                                if old_vals == new_vals and old_status == new_status:
                                    continue 
                            ...
                            if row_dict.get("status_type") == "in_progress":
                                ...
                                else: row_dict["started_at"] = datetime.now().strftime("%Y-%m-%d")
                                
                            # ── 解決 ID 錯誤：把 ID 抽出來，不傳進去更新 ──
                    ...
                    st.success(f"✅ 極速儲存完成！本次共更新了 {saved} 筆變更。")
                    refresh()
    ...
    with st.expander("📄 匯出 PDF (中文修復版)"):
        if st.button("產生 PDF"):
            try:
                from fpdf import FPDF
                import tempfile, os, urllib.request

                # ── 改用 Google 官方字型，徹底解決亂碼 ──
                font_path = "/tmp/NotoSansTC-Regular.ttf"
                if not os.path.exists(font_path):
                    with st.spinner("首次使用：下載中文字型中（約5秒）..."):
                        urllib.request.urlretrieve("https://github.com/google/fonts/raw/main/ofl/notosanstc/NotoSansTC-Regular.ttf", font_path)
                ...
                st.error(f"PDF 失敗 (請確認 requirements.txt 有 fpdf2)：{e}")
    ...
    if df_all.empty:
        ...
        if df_ana.empty: st.info("此條件下沒有資料")
            ...
            with c1:
                ...
                status_counts["狀態"] = status_counts["status_type"].map({k:v["label"] for k,v in STATUS_CONFIG.items()})
                st.bar_chart(status_counts.set_index("狀態")["數量"], color="#2196f3", use_container_width=True)
            ...
            with c2:
                ...
                if not df_pct.empty:
                    bins = [0,25,50,75,100]; labels = ["0-25%","26-50%","51-75%","76-100%"]
            ...
            with c3:
                ...
                if not client_cnt.empty: st.bar_chart(client_cnt, color="#ff7043", use_container_width=True)
                else: st.info("無業主資料")
            ...
            if df_inprog.empty: st.info("目前沒有製作中的工程")
            else:
                show_cols = ["project_name","client","section","completion","tracking"]
                show_names = {"project_name":"工程名稱","client":"業主","section":"分區","completion":"完成率","tracking":"備註"}
            ...
            if df_pending.empty: st.info("目前沒有待交站的工程")
            else:
                show_cols2 = ["project_name","client","section","handover","handover_year","contact"]
                show_names2 = {"project_name":"工程名稱","client":"業主","section":"分區","handover":"交站","handover_year":"年份","contact":"對應窗口"}
...
with page_tab3:
    ...
    if PLOTLY_OK:
        ...
        def apply_font(fig):
            fig.update_layout(font=dict(family=FONT, size=13), title_font=dict(family=FONT, size=15), legend=dict(font=dict(family=FONT)), paper_bgcolor="white", plot_bgcolor="#f8faff")
            return fig
        ...
        st.markdown("""
        <div style="background:linear-gradient(135deg,#0a2540,#1a6b3c); padding:12px 20px;border-radius:8px;margin-bottom:14px;">
        ...
        if df_wl.empty:
            st.warning("⚠️ 尚無工時資料，請至下方「新增工時記錄」添加第一筆資料。")
        else:
            st.markdown("#### 📅 查詢區間")
            d1, d2, d3, d4 = st.columns([1.5, 1.5, 1.5, 1.5])
            min_date = df_wl["start_time"].dt.date.min()
            max_date = df_wl["start_time"].dt.date.max()
            with d1: date_from = st.date_input("開始日期", value=min_date, min_value=min_date, max_value=max_date)
            with d2: date_to   = st.date_input("結束日期", value=max_date, min_value=min_date, max_value=max_date)
            with d3:
            ...
            if df_f.empty: st.warning("此條件無資料，請調整篩選範圍。")
                ...
                df_f["超時"] = df_f["效率比%"].apply(lambda x: True if (pd.notna(x) and x > 120) else False)
                ...
                with row1_l:
                    st.markdown("#### 👷 人員累計工時")
                    op_hours = (df_f.groupby("operator")["actual_hours"].sum().reset_index().rename(columns={"operator":"人員","actual_hours":"累計工時(h)"}).sort_values("累計工時(h)", ascending=False))
                    fig1 = px.bar(op_hours, x="人員", y="累計工時(h)", color="累計工時(h)", color_continuous_scale="Blues", text="累計工時(h)")
                ...
                with row1_r:
                    st.markdown("#### 🔧 工序工時佔比")
                    proc_hours = (df_f.groupby("process_name")["actual_hours"].sum().reset_index().rename(columns={"process_name":"工序","actual_hours":"工時(h)"}))
                    fig2 = px.pie(proc_hours, names="工序", values="工時(h)", hole=0.35, color_discrete_sequence=px.colors.qualitative.Set3)
                ...
                df_daily = (df_f.groupby(df_f["start_time"].dt.date).agg(actual=("actual_hours","sum"), standard=("standard_hours","sum")).reset_index().rename(columns={"start_time":"日期"}))
                df_daily["效率比%"] = (df_daily["actual"] / df_daily["standard"] * 100).round(1)
                fig3 = go.Figure()
                fig3.add_trace(go.Bar(x=df_daily["日期"], y=df_daily["actual"], name="實際工時", marker_color="#2196f3"))
                fig3.add_trace(go.Bar(x=df_daily["日期"], y=df_daily["standard"], name="標準工時", marker_color="#4caf50", opacity=0.6))
                fig3.add_trace(go.Scatter(x=df_daily["日期"], y=df_daily["效率比%"], name="效率比%", yaxis="y2", mode="lines+markers", line=dict(color="#ff7043", width=2), marker=dict(size=6)))
                fig3.update_layout(barmode="group", height=350, yaxis=dict(title="工時 (小時)"), yaxis2=dict(title="效率比 (%)", overlaying="y", side="right", showgrid=False), legend=dict(orientation="h", y=1.1), xaxis_title="")
                ...
                if df_alert.empty: st.success("✅ 本期間無超時異常工單！")
                else:
                    st.error(f"🔴 共 {len(df_alert)} 筆超時工單")
                    alert_show = df_alert[["order_no","process_name","operator","start_time","actual_hours","standard_hours","效率比%","notes"]].rename(columns={"order_no":"工單編號","process_name":"工序","operator":"執行人員","start_time":"開始時間","actual_hours":"實際工時(h)","standard_hours":"標準工時(h)","notes":"備註"}).sort_values("效率比%", ascending=False)
                    alert_show["開始時間"] = alert_show["開始時間"].dt.strftime("%Y-%m-%d %H:%M")
                    def highlight_alert(row):
                        if row["效率比%"] > 150: return ["background-color:#ffcdd2"]*len(row)
                        elif row["效率比%"] > 120: return ["background-color:#fff9c4"]*len(row)
                        return [""]*len(row)
                    st.dataframe(alert_show.style.apply(highlight_alert, axis=1).format({"效率比%":"{:.1f}%"}), use_container_width=True, hide_index=True)

                st.divider()
                st.markdown("#### 🗺 人員 × 工序 工時熱力圖")
                pivot = df_f.pivot_table(values="actual_hours", index="operator", columns="process_name", aggfunc="sum", fill_value=0)
                if not pivot.empty:
                    fig4 = px.imshow(pivot, text_auto=".1f", color_continuous_scale="YlOrRd", aspect="auto")
                    fig4.update_layout(height=max(250, len(pivot)*45), xaxis_title="工序", yaxis_title="人員", coloraxis_colorbar=dict(title="工時(h)"))
                    st.plotly_chart(apply_font(fig4), use_container_width=True)

                st.divider()
                with st.expander("📋 完整工時明細"):
                    detail = df_f[["order_no","process_name","operator","start_time","end_time","actual_hours","standard_hours","效率比%","notes"]].rename(columns={"order_no":"工單編號","process_name":"工序","operator":"執行人員","start_time":"開始","end_time":"結束","actual_hours":"實際工時(h)","standard_hours":"標準工時(h)","notes":"備註"}).copy()
                    detail["開始"] = detail["開始"].dt.strftime("%Y-%m-%d %H:%M")
                    detail["結束"] = detail["結束"].dt.strftime("%Y-%m-%d %H:%M")
                    st.dataframe(detail.style.format({"效率比%":"{:.1f}%","實際工時(h)":"{:.1f}","標準工時(h)":"{:.1f}"}), use_container_width=True, hide_index=True)
