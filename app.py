...
def check_password():
    ...
    if "password_correct" in st.session_state: st.error("😕 密碼錯誤，請再試一次。")
    return False
...
<style>
  ...
  .section-header { background: linear-gradient(90deg, #0d2137, #1a3a5c); color: white; padding: 10px 16px; border-radius: 6px; font-size: 15px; font-weight: 800; margin: 14px 0 6px 0; }
  ...
  .legend-bar { display: flex; gap: 14px; flex-wrap: wrap; background: #f8f9fa; padding: 7px 14px; border-radius: 6px; margin-bottom: 8px; font-size: 12px; align-items: center; }
  .color-box { width: 13px; height: 13px; border-radius: 3px; border: 1px solid #bbb; display: inline-block; vertical-align: middle; }
</style>
...
def load_data() -> pd.DataFrame:
    ...
    for col in df.columns: df[col] = df[col].fillna("").astype(str).replace({"None":"","nan":"","NaN":"","none":""})
    return df
...
st.markdown(f'<div style="background:linear-gradient(135deg,#0a1929,#0d47a1); padding:14px 20px;border-radius:8px;margin-bottom:12px;"><div style="color:#fff;font-size:20px;font-weight:900;letter-spacing:2px;">⚙ 工程案執行進度管理系統</div><div style="color:#90caf9;font-size:12px;margin-top:3px;">更新日期：{today} ／ Supabase 雲端資料庫 ／ 多人共用</div></div>', unsafe_allow_html=True)
...
with page_tab1:
    ...
    st.markdown('<div class="legend-bar"><strong>顏色：</strong><span><span class="color-box" style="background:#FFFF99"></span> 製作中</span><span><span class="color-box" style="background:#CCE8FF"></span> 待交站</span><span><span class="color-box" style="background:#FFFFFF"></span> 未開始</span><span><span class="color-box" style="background:#FFE0B2"></span> 停工</span><span><span class="color-box" style="background:#F0F0F0"></span> 已完成</span><span style="margin-left:auto;color:#999;font-size:11px;">★ 展開「✏️ 編輯」可修改資料</span></div>', unsafe_allow_html=True)
    ...
    if not df.empty:
        ...
        if search: df = df[df["project_name"].str.contains(search,na=False) | df["case_no"].str.contains(search,na=False) | df["client"].str.contains(search,na=False) | df["contact"].str.contains(search,na=False)]
    ...
    def color_rows(row): return [f"background-color:{STATUS_CONFIG.get(row.get('status_type',''),{}).get('bg','#FFFFFF')}"] * len(row)
    ...
    for sec in sections_to_show:
        ...
        if not df_sec.empty:
            ...
            for k,cfg in STATUS_CONFIG.items():
                if int(cts2.get(k,0)): badges += f'<span style="background:{cfg["btn"]};color:{cfg["text"]};border-radius:10px;padding:1px 9px;font-size:11px;margin-left:6px;font-weight:700;">{cfg["label"]} {int(cts2.get(k,0))}</span>'
                    ...
                    for i,row in edited_df.iterrows():
                        ...
                        if row_dict.get("status_type") == "in_progress":
                            if i < len(ids):
                                ...
                                if (not old_row.empty) and (old_row["status_type"].values[0] != "in_progress"):
        ...
        if st.button("產生 PDF"):
            try:
                ...
                if not os.path.exists(font_path):
                    with st.spinner("下載字型中..."): urllib.request.urlretrieve("https://github.com/googlefonts/noto-cjk/raw/main/Sans/SubsetOTF/SC/NotoSansSC-Regular.otf", font_path)
                ...
                for sec in SECTIONS:
                    ...
                    if df_sec.empty: continue
                    pdf.add_page()
                    pdf.set_font("Chinese","B",13); pdf.set_text_color(10,35,80)
                    pdf.cell(0,9,f"【{sec}】  ({today})  共{len(df_sec)}筆",ln=True)
                    pdf.ln(1); pdf.set_font("Chinese","B",7); pdf.set_fill_color(29,71,157); pdf.set_text_color(255,255,255)
                    for h,w in zip(HEADERS,WIDTHS): pdf.cell(w,7,h,border=1,fill=True,align="C")
                    pdf.ln(); pdf.set_font("Chinese","",6.5); pdf.set_text_color(30,30,30)
                    for _,row in df_sec.iterrows():
                        pdf.set_fill_color(*PDF_BG.get(row.get("status_type",""),(255,255,255)))
                        for k,w in zip(KEYS,WIDTHS):
                            val = str(row.get(k,"") or "")
                            pdf.cell(w,6,val[:17]+"…" if len(val)>18 else val,border=1,fill=True)
                        pdf.ln()
                with tempfile.NamedTemporaryFile(delete=False,suffix=".pdf") as tmp:
                    pdf.output(tmp.name)
                    with open(tmp.name,"rb") as f: pdf_bytes=f.read()
                    os.unlink(tmp.name)
                st.download_button("⬇ 下載 PDF", pdf_bytes, file_name=f"工程進度_{datetime.now().strftime('%Y%m%d')}.pdf", mime="application/pdf")
...
with page_tab2:
    if df_all.empty: st.warning("尚無資料")
        ...
        if df_ana.empty: st.info("無資料")
        else:
            c1, c2 = st.columns(2)
            with c1:
                st.markdown("#### 🔵 各狀態分佈")
                st_counts = df_ana["status_type"].value_counts().reset_index()
                st_counts.columns = ["type","數量"]
                st_counts["狀態"] = st_counts["type"].map({k:v["label"] for k,v in STATUS_CONFIG.items()})
                st.bar_chart(st_counts.set_index("狀態")["數量"], color="#2196f3", use_container_width=True)
            with c2:
                st.markdown("#### 📈 完成率分佈")
                df_ana["_pct"] = df_ana["completion"].apply(lambda s: float(str(s).replace("%","").strip()) if pd.notna(s) and str(s).replace("%","").strip().isdigit() else None)
                df_pct = df_ana[df_ana["_pct"].notna()].copy()
                if not df_pct.empty:
                    df_pct["區間"] = pd.cut(df_pct["_pct"], bins=[0,25,50,75,100], labels=["0-25%","26-50%","51-75%","76-100%"], include_lowest=True)
                    st.bar_chart(df_pct["區間"].value_counts().sort_index(), color="#4caf50", use_container_width=True)

            st.divider()
            st.markdown("#### 🔧 各工序完成數量")
            proc_done = {n: df_ana[c].apply(lambda x: 1 if str(x).strip() not in ["","None","nan","-"] else 0).sum() for c,n in zip(PROCESS_COLS, PROCESS_NAMES) if c in df_ana.columns}
            df_proc = pd.DataFrame(list(proc_done.items()), columns=["工序","完成數量"])
            ...
            with c3:
                st.markdown("#### 🏢 各業主工程數量")
                st.bar_chart(df_ana[df_ana["client"]!=""]["client"].value_counts().head(10), color="#ff7043", use_container_width=True)
            with c4:
                st.markdown("#### 📦 各分區平均完成率")
                sec_avg = df_ana.groupby("section")["_pct"].mean().dropna().round(1)
                st.bar_chart(sec_avg, color="#9c27b0", use_container_width=True)

            st.divider()
            st.markdown("#### ⚙ 製作中工程 — 完成率排行")
            df_inprog = df_ana[df_ana["status_type"]=="in_progress"].copy()
            if df_inprog.empty: st.info("無")
            else: st.dataframe(df_inprog.sort_values("_pct", ascending=False)[["project_name","client","section","completion","tracking"]].rename(columns={"project_name":"工程名稱","client":"業主","section":"分區","completion":"完成率","tracking":"備註"}), use_container_width=True, hide_index=True)
